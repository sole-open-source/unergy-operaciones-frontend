<template>
  ...
  <div class="em-panel" :class="{ 'em-panel--with-drawer': drawerInf }">
      ...
      <!-- ══ Acciones masivas (cuando aplica) ════════════════════ -->
            ...
            <tr v-for="inf in filtrados" :key="inf.id"
              ...
              <td class="em-td-acciones" @click.stop>
                <button class="em-icon-btn em-btn-edit" @click="editar(inf)"
                        v-tooltip.bottom="'Editar / ver informe completo'">
      ...
      <transition name="slide-right">
        <aside v-if="drawerInf" class="em-drawer">
          <header class="em-drawer-head">
            ...
            <button class="em-close" @click="cerrarDrawer">✕</button>
          </header>
          ...
          <div class="em-drawer-body">
            ...
            <div v-else-if="drawerTab === 'comentarios'" class="em-coms-wrap">
              ...
              <div v-if="drawerInf.estado !== 'aprobado'" class="em-com-add">
                <textarea v-model="nuevoComentario" rows="3"
                          :placeholder="permisoVerificar
                            ...
                            : 'Sólo el verificador puede agregar observaciones. Tú puedes responder marcando como subsanado los comentarios existentes.'"
                          class="em-textarea"
                          :disabled="!permisoVerificar" />
              ...
              <div v-else class="em-aprobado-msg">
                ✅ Informe aprobado/verificado · ya no se aceptan más observaciones (reabre el informe si necesitas modificarlo)
              </div>
            </div>
...
async function abrirDrawer(inf, tab = 'preview') {
  ...
  try {
    ...
    // Merge updated comentarios al state local
...
function imprimirDetalle() {
  ...
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${drawerInf.value.proyecto_nombre || ''}</title>
    <style>body{margin:0;font-family:'Sora',system-ui,sans-serif}</style></head><body>${detalleHtml.value}</body></html>`)
  w.document.close()
  w.focus()
...
async function verificarYAprobar() {
  ...
  try {
    // Si está en borrador, primero a revisado, luego a aprobado.
...
async function reabrir() {
  ...
  try {
    // aprobado → revisado no está permitido, hay que ir a borrador via revisado.
    // El backend permite aprobado → revisado? No, revisado → borrador sí. Aprobado no tiene transición.
    // Workaround: usar el POST upsert para re-guardar como borrador? Mejor: añadir endpoint backend.
    // Por ahora: indicamos al backend que cambie a 'borrador' directamente (puede que requiera cambio backend).
...
.em-kpi {
  ...
  background: transparent;
  border: none; border-right: 1px solid #ECE7F2;
  padding: 10px 8px;
  cursor: pointer;
  font-family: inherit;
...
.em-kpi::after {
  ...
  border-radius: 2px 2px 0 0; opacity: 0; transition: opacity .15s;
}
.em-kpi--on::after { opacity: 1; }
.em-kpi--on { background: #FAF8FE; }
.em-kpi-num { font-size: 18px; font-weight: 800; line-height: 1; color: #1A1025; }
.em-kpi-lbl { font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: .4px; color: #6B5A8A; }
.em-kpi::after { background: #915BD8; }
.em-kpi-pend::after  { background: #D97706; }   .em-kpi-pend  .em-kpi-num { color: #D97706; }
.em-kpi-coms::after  { background: #DC2626; }   .em-kpi-coms  .em-kpi-num { color: #DC2626; }
.em-kpi-resol::after { background: #2563EB; }   .em-kpi-resol .em-kpi-num { color: #2563EB; }
.em-kpi-ver::after   { background: #16A34A; }   .em-kpi-ver   .em-kpi-num { color: #16A34A; }
.em-kpi-env::after   { background: #6D28D9; }   .em-kpi-env   .em-kpi-num { color: #6D28D9; }
...
.em-table thead th {
  ...
  color: #6B5A8A; white-space: nowrap;
}
.em-col-estado { width: 130px; }
.em-col-acciones { width: 140px; text-align: right; }
.em-row {
  border-bottom: 1px solid #F3F0F9; cursor: pointer;
  transition: background .12s;
}
...
.em-drawer-title { font-size: 14px; font-weight: 800; color: #1A1025; }
.em-drawer-sub {
  font-size: 11px; color: #6B5A8A; margin-top: 4px;
  display: inline-flex; align-items: center; gap: 6px; flex-wrap: wrap;
}
.em-drawer-head .em-close {
  background: #fff; border: 1px solid #ECE7F2; color: #6B5A8A;
}
.em-drawer-head .em-close:hover { background: #F4F1FA; }

.em-drawer-tabs {
  display: flex; gap: 0; border-bottom: 1px solid #ECE7F2; padding: 0 14px;
  background: #fff;
}
.em-drawer-tab {
  position: relative;
...
.em-textarea {
  width: 100%;
  border: 1.5px solid #E5E2EC; border-radius: 8px;
  padding: 8px 10px; font-family: inherit; font-size: 12px;
  color: #2C2039; outline: none;
  resize: vertical;
  background: #fff;
}
...
.em-com-add {
  background: #FAF8FE; border: 1px dashed #DAD3EA; border-radius: 10px;
  padding: 12px;
}
