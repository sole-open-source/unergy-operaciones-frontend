      ...
      <TabPanel header="Fallas">
        <div class="space-y-4 pt-2">
          ...
          <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
            <button class="kpi-card" :class="{ 'kpi-card--active': !filtroEstado && !filtroPrioridad }"
              @click="limpiarFiltros" :style="kpiActiveStyle('#915BD8', !filtroEstado && !filtroPrioridad)">
              <span class="kpi-val">{{ todasLasFallas.length }}</span>
              <span class="kpi-lbl">Total</span>
              <span class="kpi-bar" style="background:#915BD8" />
            </button>
            <button class="kpi-card" :class="{ 'kpi-card--active': filtroEstado === 'abierta' }"
              @click="setFiltroEstado('abierta')" :style="kpiActiveStyle('#dc2626', filtroEstado === 'abierta')">
              <span class="kpi-val" style="color:#dc2626">{{ kpis.abierta }}</span>
              <span class="kpi-lbl">Abiertas</span>
              <span class="kpi-bar" style="background:#dc2626" />
            </button>
            <button class="kpi-card" :class="{ 'kpi-card--active': filtroEstado === 'en_gestion' }"
              @click="setFiltroEstado('en_gestion')" :style="kpiActiveStyle('#ea580c', filtroEstado === 'en_gestion')">
              <span class="kpi-val" style="color:#ea580c">{{ kpis.en_gestion }}</span>
              <span class="kpi-lbl">En gestión</span>
              <span class="kpi-bar" style="background:#ea580c" />
            </button>
            <button class="kpi-card" :class="{ 'kpi-card--active': filtroEstado === 'en_espera' }"
              @click="setFiltroEstado('en_espera')" :style="kpiActiveStyle('#2563eb', filtroEstado === 'en_espera')">
              <span class="kpi-val" style="color:#2563eb">{{ kpis.en_espera }}</span>
              <span class="kpi-lbl">En espera</span>
              <span class="kpi-bar" style="background:#2563eb" />
            </button>
            <button class="kpi-card" :class="{ 'kpi-card--active': filtroPrioridad === 'critica' }"
              @click="setFiltroPrioridad('critica')" :style="kpiActiveStyle('#dc2626', filtroPrioridad === 'critica')">
              <span class="kpi-val" style="color:#dc2626">{{ kpis.critica }}</span>
...
// Opciones del selector vienen de `proyectos` (disponible desde el inicio,
// sin esperar a que termine la carga de generación).
...
watch(activeTab, (val) => {
  if (val === 1) {
    if (!proyectos.value.length) cargarProyectos()
    if (!genHistorial.value.length) cargarGeneracion()
  }
